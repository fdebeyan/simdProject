...
int main(int argc, char *argv[]) {
    ...
    if (Threads <= 1) {
        for (unsigned i = firstInputFile; i < inputFiles.size(); i += 2) {
            if ((i + 1) < inputFiles.size()) {
                unsigned j = (i + 1);
//                std::cout << i << "__" << j << std::endl;
//                std::cout << inputFiles[i] << inputFiles[j] << std::endl;
                grepEngine.doGrep(inputFiles[i], inputFiles[j]);
