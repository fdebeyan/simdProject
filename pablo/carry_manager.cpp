...
Value * CarryManager::addCarryInCarryOut(const unsigned localIndex, Value * const e1, Value * const e2) {
    ...
    if (mBitBlockWidth == 128) {
        ...
        sum = iBuilder->simd_add(64, digitsum, carryq_value);
        Value * digitcarry = iBuilder->simd_or(carrygen, iBuilder->simd_and(carryprop, iBuilder->CreateNot(sum)));
        ...
        setCarryOut(localIndex, digitcarry);
...
//        std::cerr << "shortAdvance index = " << index << ", shiftAmount = " << shiftAmount << "\n";
        result = iBuilder->simd_or(iBuilder->simd_srli(DSSLI_FIELDWIDTH, carryIn, DSSLI_FIELDWIDTH - shiftAmount), iBuilder->simd_slli(DSSLI_FIELDWIDTH, value, shiftAmount));
    } else {
...
void CarryManager::setCarryOut(const unsigned localIndex, Value * carryOut) {
    ...
    if (mBitBlockWidth < 256) { // #ifndef USING_LONG_ADD
        Value * carry_bit = iBuilder->CreateLShr(carryOut, (mBitBlockWidth / 2)-1);
